...
public abstract class BlockStateBaseMixin {
    ...
    /**
     * Allows entities to stand on fluids, thank you <a href="https://github.com/florensie/ExpandAbility/blob/master/common/src/main/java/be/florens/expandability/mixin/fluidcollision/BlockStateBaseMixin.java">ExpandAbility</a>
     */
    ...
    private VoxelShape addFluidCollision(VoxelShape originalShape, BlockGetter level, BlockPos pos, CollisionContext context) {
        ...
        if (context.isAbove(fluidShape.move(0, -stepHeight, 0), pos, false)) {
            VoxelShape defaultCombined = Shapes.or(fluidShape, originalShape); // default shape
            FluidCollisionEvent event = new FluidCollisionEvent(entity, pos, fluidState, defaultCombined);
            NeoForge.EVENT_BUS.post(event);

            if (event.isCanceled()) {
                return event.getResultShape(); // return the modified shape
            }
