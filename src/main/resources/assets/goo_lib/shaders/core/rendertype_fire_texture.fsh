...
vec3 bumpMap(vec2 uv, vec2 texSize) {
    vec2 s = 1.0 / texSize;
    float p = fbm(uv);
...
void main() {
    ...
    // get dynamic texture size
    vec2 currentTexSize = vec2(textureSize(Sampler0, 0));
    vec2 pixelatedUV = floor(texCoord0 * currentTexSize) / currentTexSize;

    // compute fire distortion
    vec3 normal = bumpMap(pixelatedUV * vec2(1.0, 0.3) + MOVEMENT_DISTORTION * TIMESCALE, currentTexSize);
    vec2 displacement = clamp((normal.xy - 0.5) * DISTORTION_STRENGTH, -1.0, 1.0);

    // mix
    vec2 finalUV = pixelatedUV + displacement;

    // repixelate
    finalUV = floor(finalUV * currentTexSize) / currentTexSize;
    ...
    // multiply colors by java vertex colors
    // then only multiply with java vertex alpha
